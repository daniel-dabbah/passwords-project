...
def plot_password_length_histogram(length_percentages,dataset_name):
    ...
    ax.bar(lengths, percentages, color='blue', alpha=0.7)
    ...
    # Save the plot
    save_plot(dataset_name, "plot_password_length_histogram")
...
def plot_ascii_usage_histogram(ascii_counts,dataset_name):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(chars, percentages, color='green', alpha=0.7)
    ax.set_xlabel('ASCII Character')
    ax.set_ylabel('Percentage')
    ax.set_title('ASCII Character Usage in Passwords')
    ax.grid(axis='y', linestyle='--', alpha=0.7)
