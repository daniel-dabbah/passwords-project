...
def plot_year_usage(loaded_statistics):
    ...
    st.write("""
    The histogram of years in passwords reveals interesting trends:
    
    1. Years become increasingly popular in recent decades, possibly reflecting the growing age distribution of internet users, with younger generations more prevalent online.
    2. There's a noticeable scarcity of passwords using years from 1940-1960. This aligns with the fact that older generations tend to use online media less frequently than younger ones.
    3. The majority of years used fall between 1960-2010. We can hypothesize that these often correspond to users' birth years or other significant personal dates.
    
    This analysis provides insights into user behavior and demographics, highlighting how personal information often influences password creation.
    """)
...
def plot_entropy_distribution(loaded_statistics):
    ...
    st.write("""Entropy measures the unpredictability of passwords. Higher entropy indicates stronger passwords.
             Entropy is a crucial concept in password security, representing the measure of unpredictability or randomness in a password. It is calculated based on the length of the password and the diversity of characters it contains. The character set size is determined by the types of characters present in the password, such as lowercase letters (26 characters), uppercase letters (26 characters), numbers (10 characters), and any special symbols (which can vary in size). A larger character set increases the password's potential combinations, making it harder to guess. The length of the password also plays a crucial role, as longer passwords offer more possible combinations. The formula for calculating entropy is: Entropy = Password Length × log₂(Character Set Size). The logarithm base 2 indicates the number of bits needed to represent the total character set. For example, a password that contains lowercase letters, uppercase letters, and numbers with a total length of 8 characters has a character set size of 62. Using the formula, its entropy would be approximately 8 × log₂(62), resulting in about 47.63 bits of entropy. This value reflects the strength and unpredictability of the password based on the number of possible combinations of characters. To ensure robust security, it is recommended that a password achieve at least 80 bits of entropy, typically requiring a length of at least 12 characters that utilize a blend of all character types.

In the graph below, we can observe a curve-like distribution, which aligns with our understanding of password strength. The curve shows that there are fewer passwords with very low or very high entropy, while the majority fall in the middle range. This distribution makes sense because:

1) Very weak passwords (low entropy) are less common as users are often encouraged to create stronger passwords.
2) The peak of the curve represents the most common level of password complexity.
3) The tail extending to higher entropy values represents the strongest, most complex passwords, which are naturally less frequent.

This curve effectively illustrates the spectrum of password strengths in our dataset, from weak to strong.

We can also see a spike on the right side, which may be explained by passwords that are highly randomized and not similar to any other passwords. These could potentially be generated by password generators, resulting in very high entropy values.""")
...
def static_visualization_page():
    ...
    # New plots
    st.write("""
    Below, we present visualizations showing the average number of characters for each category:
    - Numbers
    - Upper Case Letters
    - Lower Case Letters
    - Special Characters (!@#$%^&*()-_=+[]{|;:'\,.<>?/`~)

    These graphs display how the average count of each character type changes as password length increases. 
    This information can provide insights into common password composition patterns and how they vary with length.
    """)
    plot_average_numbers_by_length(loaded_statistics, 'Numbers Characters')
